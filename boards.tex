% ============================================================
%  AND gate - board-level wiring diagram (ALTERNATIVE build from
%  a finished NAND board + a finished NOT board; the compact
%  single-breadboard build is in wiring.tex):
%        A,B -> [NAND] -> (A.B)-bar -> [NOT] -> Y = A.B
%  Build the NAND board and the NOT board, share one +5 V rail and
%  one GND, then wire them as below.
%  Compile:  pdflatex boards.tex
% ============================================================
...
\begin{tikzpicture}[line width=1.1pt, font=\sffamily]
  ...
  % ---------- the two boards ----------
  \draw[rounded corners=3pt, fill=blue!6] (2.6,-1.6) rectangle (6.6,1.6);
  \node[align=center, font=\bfseries] at (4.6,0.4) {NAND gate board};
  \node[align=center, font=\footnotesize] at (4.6,-0.55) {2$\times$2N3904 + 2$\times$2N3906};

  \draw[rounded corners=3pt, fill=orange!10] (9.8,-1.6) rectangle (13.4,1.6);
  ...
  % ---------- NAND -> NOT ----------
  \draw (6.6,0) -- (9.8,0);
  \node[above,font=\footnotesize] at (8.2,0.1) {$\overline{A\cdot B}$};

  % ---------- output ----------
  \draw (13.4,0) -- (15.7,0) node[ocirc]{} node[right]{Output $Y=A\cdot B$};
